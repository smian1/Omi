...
def set_action_item_status(data: Union[SetMemoryActionItemsStateRequest, UpdateActionItemRequest], memory_id: str,
                           uid=Depends(auth.get_current_user_uid)):
    memory = _get_memory_by_id(uid, memory_id)
    memory = Memory(**memory)
    action_items = memory.structured.action_items
    
    if isinstance(data, UpdateActionItemRequest):
